...
-- 2024-11-24,MOsiechko,Created File
--**************************************************************************--
Begin Try
	Use Master;
	If Exists(Select Name From SysDatabases Where Name = 'Assignment07DB_MOsiechko')
	 Begin 
	  Alter Database [Assignment07DB_MOsiechko] set Single_user With Rollback Immediate;
	  Drop Database Assignment07DB_MOsiechko;
	 End
	Create Database Assignment07DB_MOsiechko;
End Try
Begin Catch
	Print Error_Number();
End Catch
go
Use Assignment07DB_MOsiechko;
...
ORDER BY ProductName, CAST([InventoryDate] AS DATE), [Count];     --CAST converts InvDate into DATE and sorts it by month, rather than alphabetically
GO
...
AS 
SELECT TOP 1000000					--Need this for ORDER BY CLAUSE
	P.ProductName
   ,InventoryDate = DateName(mm, I.InventoryDate) +' ,' +DateName(yyyy, I.InventoryDAte)
   ,I.[Count] AS InventoryCount
FROM vProducts AS P
...
SELECT TOP 1000000
   ...
   ,InventoryCount
   ,PreviousMonthCount 
   ,[CountVsPreviousChangeKPI]= IsNull(CASE
...
RETURN (SELECT TOP 1000000
   ...
   ,InventoryCount
   ,PreviousMonthCount
   ,CountVsPreviousChangeKPI
FROM vProductInventoriesWithPreviousMonthCountsWithKPIs
WHERE [CountVsPreviousChangeKPI] = @KPIValue
